...
export class TimestampsModalComponent implements OnInit {
  ...
  readonly privateUrl: string | null = `${location.href}/${this.urlTo.transform(this.context.data?.category?.secret_desc ?? this.context.data?.category?.secret ?? ``, 'menuCategory.private')}`;
  readonly publicUrl: string | null = `${location.href}/${this.urlTo.transform(this.context.data?.category?.id ?? ``, 'menuCategory.public')}`;
